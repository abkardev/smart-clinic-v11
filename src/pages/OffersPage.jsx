...
// Vite uses import.meta.env.VITE_* — derive the base server URL from it
const API_BASE = (() => {
...
export default function OffersPage() {
  ...
  const handleOpen = (offer=null) => {
    if (offer) {
      ...
      setPreview(offer.imageUrl ? `${API_BASE}${offer.imageUrl}` : offer.imageBase64||'');
      setEditId(offer.id);
            ...
            <Paper elevation={0} sx={{ borderRadius:3, overflow:'hidden', border:'1px solid rgba(148,163,184,0.15)', transition:'all 0.2s', '&:hover':{ transform:'translateY(-3px)', boxShadow:'0 12px 32px rgba(0,0,0,0.1)' } }}>
              ...
              <Box sx={{ height:180, bgcolor:'#F1F5F9', position:'relative', overflow:'hidden' }}>
                {offer.imageUrl||offer.imageBase64 ? (
                  <Box component="img" src={offer.imageUrl?`${API_BASE}${offer.imageUrl}`:offer.imageBase64} alt={offer.titleEn} sx={{ width:'100%', height:'100%', objectFit:'cover' }}/>
                ) : (
