...
export default function UsersPage() {
  ...
  return (
    <Box>
      ...
      <Paper elevation={0} sx={{ height: 540, borderRadius: 3, overflow: 'hidden', border: '1px solid rgba(148,163,184,0.15)' }}>
        <DataGrid
          // KEY FIX: changing key forces DataGrid to fully re-render all renderCell outputs
          // when any role changes. Without this, the Select inside renderCell shows stale value
          // because DataGrid virtualises rows and doesn't re-call renderCell on state update.
          getRowId={(row) => row.id}
          rows={users}
          columns={columns}
          loading={loading}
